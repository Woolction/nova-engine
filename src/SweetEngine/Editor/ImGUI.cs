...
public unsafe static class ImGUI
{
    ...
    public static void Init(in EngineContext context)
    {
        ...
        SetupFonts(io, in context.Resource->TextureLoader);
        SetupBuffers();
    }

    public static void Update(in EngineContext context)
    {
        UpdateIO(context.Device);
    ...
    {
        context.Resource->ShaderLoader.Use(shader);

        EnableBuffers();

        context.Resource->ShaderLoader.SetInt(shader, "uTexture", 0);

        Matrix4x4 ortho = Matrix4x4.CreateOrthographicOffCenter(
            0, context.Device->Window.Size.X,
            context.Device->Window.Size.Y, 0,
            -1, 1
        );

        context.Resource->ShaderLoader.SetMatrix4(shader, "uProj", &ortho, false);

        ImGui.Render();

        RenderDrawData(ImGui.GetDrawData(), context.Device);

        DisableBuffers();
    }

    private static void EnableBuffers()
    {
    ...
    private static void DisableBuffers()
    {
    ...
    private static void RenderDrawData(ImDrawDataPtr drawData, Device* device)
    {
    ...
    private static void UpdateIO(Device* device)
    {
        ...
        GraphicContext.Glfw.SetCharCallback(GraphicContext.Window, (wnd, codepoint) =>
        {
            io.AddInputCharacter(codepoint);
        });
    }

    private static void SetupFonts(ImGuiIOPtr io, in TextureLoader textureLoader)
    {
